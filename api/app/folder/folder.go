...
type Folder interface {
	// Create adds a new folder to the portfolio. Position is automatically
	// assigned as max(existing) + 1.
	...
	// Update renames or updates the icon/image of a folder.
	Update(ctx context.Context, id uuid.UUID, req model.UpdateFolderRequest) (model.Folder, error)
...
// Get fetches a single folder by ID.
func (s *service) Get(ctx context.Context, id uuid.UUID) (model.Folder, error) {
	return s.dp.FolderStore.GetFolderByID(ctx, id)
}
...
func (s *service) Update(ctx context.Context, id uuid.UUID, req model.UpdateFolderRequest) (model.Folder, error) {
	log := siloLogger.FromCtx(ctx).With().
		Str(siloLogger.LogStrKeyMethod, "folder.Update").
		Logger()

	folder, err := s.dp.FolderStore.GetFolderByID(ctx, id)
	if err != nil {
		return model.Folder{}, err
	}
...
// Delete removes a folder; assets inside it become folder-less automatically.
func (s *service) Delete(ctx context.Context, id uuid.UUID) error {
	return s.dp.FolderStore.DeleteFolder(ctx, id)
}
